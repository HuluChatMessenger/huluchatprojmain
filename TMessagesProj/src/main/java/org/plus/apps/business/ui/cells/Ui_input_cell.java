...
public class Ui_input_cell extends FrameLayout {

    private EditTextBoldCursor textView;
    private SimpleTextView deleteImageView;
    private SimpleTextView moveImageView;
    ...
    public Ui_input_cell(Context context, boolean caption,String prefix,String suffix){
        super(context);
        ...
        if (!ShopUtils.isEmpty(suffix) || !ShopUtils.isEmpty(prefix)) {

            addView(textView, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.WRAP_CONTENT, (LocaleController.isRTL ? Gravity.RIGHT : Gravity.LEFT) | Gravity.CENTER_VERTICAL, !ShopUtils.isEmpty(prefix)?LocaleController.isRTL ? 58 : 64:19, 0, !ShopUtils.isEmpty(suffix)?!LocaleController.isRTL ? 58 : 64:19, 0));

            if(!ShopUtils.isEmpty(prefix)){
                moveImageView = new SimpleTextView(context);
                moveImageView.setFocusable(false);
                moveImageView.setTextSize(13);
                moveImageView.setText(prefix);
                moveImageView.setGravity(Gravity.CENTER);
                moveImageView.setTextColor(Theme.getColor(Theme.key_dialogTextBlack));
                addView(moveImageView, LayoutHelper.createFrame(30, 30, (LocaleController.isRTL ? Gravity.RIGHT : Gravity.LEFT) | Gravity.TOP, 6, 2, 6, 0));

            }

            if(suffix != null){
                deleteImageView = new SimpleTextView(context);
                deleteImageView.setFocusable(false);
                deleteImageView.setText(suffix);
                deleteImageView.setTextSize(13);
                deleteImageView.setGravity(Gravity.CENTER);
                deleteImageView.setTextColor(Theme.getColor(Theme.key_dialogTextBlack));
                addView(deleteImageView, LayoutHelper.createFrame(30, 30, (LocaleController.isRTL ? Gravity.LEFT : Gravity.RIGHT) | Gravity.TOP, LocaleController.isRTL ? 3 : 0, 0, LocaleController.isRTL ? 0 : 3, 0));
            ...
            checkBox.setOnClickListener(v -> {
                ...
                onCheckBoxClick(Ui_input_cell.this, !checkBox.isChecked());
            });
    ...
    protected void onCheckBoxClick(Ui_input_cell editText, boolean checked) {
        checkBox.setChecked(checked, true);
    }

    protected boolean isChecked(Ui_input_cell editText) {
        return false;
    }
