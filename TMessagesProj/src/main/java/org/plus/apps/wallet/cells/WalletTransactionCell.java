...
public class WalletTransactionCell extends LinearLayout {
    ...
    private WalletDataSerializer.Transaction currentTransaction;
    ...
    public WalletTransactionCell(Context context) {
        ...
        encryptedImageView.setImageResource(R.drawable.lock_close);
...
//    }
    ...
    public WalletDataSerializer.Transaction getCurrentTransaction() {
    ...
    public void setTransaction(WalletDataSerializer.Transaction transaction, boolean divider) {
        ...
        long value = 0;
        String message = transaction.reason;

        LayoutParams layoutParams = (LayoutParams) fromTextView.getLayoutParams();
        ...
        CharSequence text = WalletUtils.formatCurrencyEmpty(transaction.amount);
        isEmpty = false;
        boolean isPending = transaction.status.equals("REQUESTED");

        currentAmount = value;
        currentDate = Instant.parse(transaction.created_at).toEpochMilli();
        int index = TextUtils.indexOf(text, '.');

        valueTextView.setText(text);
        dateTextView.setText(LocaleController.getInstance().formatterDay.format(currentDate));

            if (isPending) {
                clockImage.setVisibility(VISIBLE);
                currentTransactionFee = 0;
                feeTextView.setVisibility(INVISIBLE);
            } else {
                clockImage.setVisibility(GONE);
               // currentTransactionFee = transaction.transaction_fee;
            ...
            layoutParams = (LayoutParams) addressLinearLayout.getLayoutParams();
            ...
            if (commentTextView.getVisibility() == VISIBLE) {
                layoutParams = (LayoutParams) commentTextView.getLayoutParams();
