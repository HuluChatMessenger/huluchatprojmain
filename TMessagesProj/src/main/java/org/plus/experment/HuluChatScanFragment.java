...
import static org.plus.apps.business.ShopUtils.isHuluChatSupportedLink;

@TargetApi(18)
public class HuluChatScanFragment extends BaseFragment implements Camera.PreviewCallback {
        ...
        BottomSheet bottomSheet = new BottomSheet(parentFragment.getParentActivity(), false) {
            {
                ...
                HuluChatScanFragment fragment = new HuluChatScanFragment(TYPE_QR) {
                    @Override
    ...
    public HuluChatScanFragment(int type) {
        super();
        ...
        AndroidUtilities.runOnUIThread(new Runnable() {
            ...
            public void run() {
                if (cameraView != null && !recognized && cameraView.getCameraSession() != null) {
                    cameraView.getCameraSession().setOneShotPreviewCallback(HuluChatScanFragment.this);
    ...
    private String tryReadQr(byte[] data, Size size, int x, int y, int side, Bitmap bitmap) {
        try {
            ...
            if(!isHuluChatSupportedLink(text)){
                return null;
            }
